...
export function generateReceiptHash(receipt: {
  ...
  const dateStr = receipt.executedAt instanceof Date ? receipt.executedAt.toISOString() : String(receipt.executedAt);
  const inputStr = `${receipt.receiptId}${receipt.taskId}${receipt.status}${dateStr}`;
  try {
    const cryptoModule = require("crypto");
    return cryptoModule.createHash("sha256").update(inputStr).digest("hex");
  } catch (e) {
    let hash = 0;
