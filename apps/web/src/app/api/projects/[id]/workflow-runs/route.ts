...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const runs = await prisma.workflowRun.findMany({ where: { workspaceId: ctx.workspaceId, OR: [{ inputContext: { path: "$.projectId", equals: id } } as any, { input: { contains: id } }] }, orderBy: { createdAt: "desc" }, take: limit })
