...
export async function GET(request: Request) {
  ...
  try {
    const result = await evaluateCanaryHealth(undefined, {
      ...
      getLatestMetrics: async (workspaceId: string, agentId: string) => { const oneDayAgo = new Date(Date.now() - 86400000); const logs = await prisma.agentLog.findMany({ where: { workspaceId, agentId, createdAt: { gte: oneDayAgo } } }); const total = logs.length; if (total === 0) return { errorRate: 0, successRate: 1.0, avgLatencyMs: 0, humanCorrectionRate: 0, connectorSuccessRate: 1.0 }; const errors = logs.filter((l: any) => l.status === 'failed' || l.status === 'error').length; return { errorRate: errors / total, successRate: (total - errors) / total, avgLatencyMs: 0, humanCorrectionRate: 0, connectorSuccessRate: 1.0 } },
    })
