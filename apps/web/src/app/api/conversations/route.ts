...
export async function POST(request: Request) {
  ...
  try {
    ...
    const conversation = await auditedWrite({ actor, action: "conversation.create", targetType: "conversation", targetId: conversationId, riskLevel: "low", automationLevel: "L2", triggeredBy: "user", workspaceId: ctx.workspaceId, detail: `创建对话: ${title}` }, async () => {
      ...
      if (messagesWithId) for (const m of messagesWithId) { if (m.trace) try { await prisma.reasoningTrace.create({ data: { traceId: m.trace.traceId, conversationId, messageId: m.id, workspaceId: ctx.workspaceId, steps: m.trace.steps, totalDurationMs: m.trace.totalDurationMs || null } }) } catch {} }
      return created
  ...
  } catch (error) { if (error instanceof ForbiddenError) return errorResponse(error.message, 403); logger.error('POST /api/conversations: 失败', { error: error instanceof Error ? error.message : '未知错误' }); return errorResponse("服务器内部错误") }
}
