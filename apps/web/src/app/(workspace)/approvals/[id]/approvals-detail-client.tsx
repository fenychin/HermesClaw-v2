...
interface ApprovalsDetailClientProps {
  initialProposal: HarnessProposal;
  id: string;
  canaryThresholds?: {
...
export default function ApprovalsDetailClient({ initialProposal, id, canaryThresholds }: ApprovalsDetailClientProps) {
  const abortThreshold = canaryThresholds?.abortErrorRate ?? 0.20;
  const router = useRouter();
  const [proposal, setProposal] = useState<HarnessProposal>(initialProposal);
  const [metrics, setMetrics] = useState<any | null>(initialProposal.status === "canary" ? (initialProposal as any).canaryMetrics : null);
  ...
  useEffect(() => {
    if (proposal.status === "canary") {
      fetchMetrics(id);
      // 30秒轮询灰度指标
      const interval = setInterval(() => fetchMetrics(id), 30000);
          ...
          <div className="bg-card/40 border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-accent/20 rounded-lg p-3 space-y-1">
                ...
                <p className={cn("text-lg font-mono font-semibold", metrics.errorRate > abortThreshold ? "text-danger animate-pulse" : "text-success")}>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs text-hint">
                <span>灰度失败率指示条（红线 ${(abortThreshold * 100).toFixed(1)}%）</span>
                <span>{(metrics.errorRate * 100).toFixed(1)}% / ${(abortThreshold * 100).toFixed(1)}%</span>
              </div>
              <div className="w-full bg-accent/35 rounded-full h-2 overflow-hidden relative">
                <div className="absolute top-0 bottom-0 w-0.5 bg-danger/50 z-10" style={{ left: `${abortThreshold * 100}%` }} title={`${(abortThreshold * 100).toFixed(1)}% 警界线`} />
                <div 
                  className={cn("h-full transition-all duration-500", metrics.errorRate > abortThreshold ? "bg-danger" : "bg-info")}
                  style={{ width: `${Math.min(metrics.errorRate * 100, 100)}%` }}
                />
              </div>
            </div>

            {metrics.totalLogs >= 5 && metrics.errorRate > abortThreshold && (
              <div className="bg-danger/10 border border-danger/20 rounded-xl p-3 flex gap-2">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-[10px] mt-0.5 leading-snug">
                    当前灰度运行次数已达 {metrics.totalLogs} 次，且失败率已达 {(metrics.errorRate * 100).toFixed(1)}%（超出 ${(abortThreshold * 100).toFixed(1)}% 红线）。
                  ...
                  <div className="bg-warning/10 border border-warning/20 rounded-xl p-3 flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground text-[10px] mt-0.5 leading-snug">
                        ...
                        建议在发布后 72 小时内针对相关 service 和 API 回调指标保持追踪。
                      </p>
                    </div>
                  </div>
                </div>
