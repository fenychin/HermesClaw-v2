...
import type { ApprovalCheckpoint } from "./page";

interface ApprovalsClientProps {
  initialCheckpoints: ApprovalCheckpoint[];
  initialProposals: HarnessProposal[];
  currentUser: {
    id: string;
    email: string;
    role: string;
...
export default function ApprovalsClient({ initialCheckpoints, initialProposals, currentUser }: ApprovalsClientProps) {
  const [activeCategory, setActiveCategory] = useState<"proposals" | "checkpoints" | "all">("proposals");
  
  // Proposals 状态
  const [proposals, setProposals] = useState<HarnessProposal[]>(initialProposals);
  ...
  // Checkpoints 状态
  const [checkpoints, setCheckpoints] = useState<ApprovalCheckpoint[]>(initialCheckpoints);
  ...
  const handleDecideCheckpoint = async (id: string, decision: "approved" | "rejected", comment?: string) => {
    ...
    try {
      const res = await fetch(`/api/approvals/${id}/decide`, {
        ...
        body: JSON.stringify({ decision, comment, decidedBy: currentUser.id }),
      });
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                {[
                  ...
                  <button
                    key={t.value}
                    onClick={() => {
                      setCheckpointFilter(t.value);
                      fetchCheckpoints(t.value);
                    }}
                    className={cn(
            ...
            {isCheckpointsLoading ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {checkpoints.map((cp) => {
                  ...
                  const isUrgent = cp.remainingMs < 7200000;
                  ...
                  const isMyRequest = cp.requestedBy === currentUser.id || cp.requestedBy === currentUser.email;
                            ...
                            <div className="flex items-center gap-2">
                              {isUserViewer ? (
                                <span className="text-[10px] text-hint flex items-center gap-1 bg-accent/20 px-2.5 py-1 rounded-lg border border-border/40 font-medium">
                                  <Shield className="size-3" /> 只读成员无权审批
                                </span>
                              ) : isUserAdmin ? (
                                <>
                                  <button
                                ...
                                <span className="text-[10px] text-hint flex items-center gap-1 bg-accent/20 px-2.5 py-1 rounded-lg border border-border/40 font-medium">
                                  <Shield className="size-3" /> 仅管理员可审批进化提案
                                </span>
                              ) : isMyRequest ? (
                                <>
                                  <button
                                ...
                                <span className="text-[10px] text-hint flex items-center gap-1 bg-accent/20 px-2.5 py-1 rounded-lg border border-border/40 font-medium">
                                  <Shield className="size-3" /> 无法审批其他人提交的工单
                                </span>
                              )}
                            </div>
                          )}
