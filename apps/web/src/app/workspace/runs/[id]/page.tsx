      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card/20 border border-border/40 rounded-xl p-4 space-y-4">
              {runStatus?.executionEvents.map((event) => (
                <div key={event.eventId} className="space-y-1 text-xs">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="font-semibold text-foreground truncate pr-2">{event.eventType}</span>
                    <span className="text-hint font-mono shrink-0">
                      {new Date(event.timestamp).toLocaleTimeString()}
                    </span>
                  </div>
                  <p className="text-[10px] text-muted-foreground leading-relaxed font-mono truncate-3-lines">
                    {event.payload.detail || "物理执行事件记录"}
                  </p>
                </div>
              ))}
