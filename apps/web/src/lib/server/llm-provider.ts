...
/**
 * 通用 LLM 非流式文本调用 —— 按 Provider 分派。
 * 消除路由层内联的 callLlmText / callDeepSeekText 重复代码。
 */
export async function callLlmText(options: CallLlmTextOptions): Promise<string> {
  const { provider, model, systemPrompt, userPrompt, maxTokens = 2048, signal } = options
  if (provider === "anthropic") {
    return callAnthropicText({ systemPrompt, userPrompt, model, maxTokens })
  }
  return callDeepSeekText({ systemPrompt, userPrompt, model, maxTokens, signal })
}

/** DeepSeek 纯文本调用参数 */
export interface CallDeepSeekTextOptions {
  systemPrompt: string
  userPrompt: string
  model?: string
  maxTokens?: number
...
/**
 * 调用 DeepSeek Chat API（纯文本，无 JSON 模式）。
 * 提取自 route.ts 的内联实现，消除各处重复的 fetch 模板。
 */
export async function callDeepSeekText(options: CallDeepSeekTextOptions): Promise<string> {
  const { systemPrompt, userPrompt, model = DEFAULT_DEEPSEEK_MODEL, maxTokens = 2048, signal } = options
