...
const ALLOWED_MIME_TYPES = new Set(["application/pdf","application/msword","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/vnd.ms-excel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.ms-powerpoint","application/vnd.openxmlformats-officedocument.presentationml.presentation","text/plain","text/csv","text/markdown","application/json","text/html","text/xml","image/png","image/jpeg","image/gif","image/webp","image/svg+xml","application/zip","application/x-rar-compressed","application/gzip"])
const ALLOWED_EXTENSIONS = new Set([".pdf",".doc",".docx",".xls",".xlsx",".ppt",".pptx",".txt",".csv",".md",".json",".html",".xml",".png",".jpg",".jpeg",".gif",".webp",".svg",".zip",".rar",".gz"])
...
export async function uploadFile(file: File, workspaceId: string, elapsed: () => string) {
  ...
  const auditEntry = await createAuditEntry({ actor, action: "file.upload", targetType: "file", targetId: fileId, riskLevel: "low", automationLevel: "L2", triggeredBy: "user", workspaceId, detail: `${file.name} (${sizeMB}MB)`, contextSnapshot: { fileName: file.name, size: file.size, type: file.type || "unknown" } })
  try {
    ...
    return { name: file.name, url, size: file.size, type: file.type || "application/octet-stream", extracted: extractResult.ok ? { ok: true, content: extractResult.content } : { ok: false, note: extractResult.note } }
  } catch (writeError) {
