...
export async function getRecentRecords(workspaceId: string, type: string, industry?: string): Promise<{ records: RecentRecordItem[] }> {
  ...
  if (type === "all" || type === "conversation") queries.push({ label: "conversation", promise: prisma.conversation.findMany({ where: { workspaceId }, orderBy: { updatedAt: "desc" }, take: LIMIT, select: { id: true, title: true, updatedAt: true } }).then(c => c.map(x => ({ id: x.id, type: "conversation" as const, title: x.title, timestamp: x.updatedAt.toISOString(), href: `/workspace/chat?load=${x.id}` }))) })
  if (type === "all" || type === "task") queries.push({ label: "task", promise: prisma.task.findMany({ where: { workspaceId }, orderBy: { updatedAt: "desc" }, take: LIMIT, select: { id: true, title: true, updatedAt: true, status: true, priority: true, projectId: true } }).then(t => t.map(x => ({ id: x.id, type: "task" as const, title: x.title, timestamp: x.updatedAt.toISOString(), href: x.projectId ? `/projects/${x.projectId}` : "/workspace/chat", meta: { status: x.status, priority: x.priority } }))) })
