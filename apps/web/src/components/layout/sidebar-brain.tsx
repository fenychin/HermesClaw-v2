...
export const SidebarBrain = memo(function SidebarBrain({
  ...
  const prewarm = (href: string) => {
    router.prefetch(href);
    prewarmWorkspaceRoute(href);
  };
      ...
      {collapsed ? (
        ...
        >
          ...
          <motion.span
            ...
            className="shrink-0 flex items-center justify-center text-muted-foreground/60"
          >
            ...
            <div className="mt-1 ml-4 space-y-0.5 border-l border-sidebar-border/40 pl-2">
              {brainNav.map((child) => {
                ...
                return (
                  ...
                  >
                    <ChildIcon className={cn("size-3.5 shrink-0", isChildActive ? "text-[#6D5EF9]" : "text-muted-foreground")} />
                    <span className="truncate flex-1 text-left">
                      {child.label}
                    </span>
                  </Link>
