...
5. `SOUL.md` 是 Hermes Agent 运行时人格与用户交互规范的定义文档，与本文件并行生效：
   - `SOUL.md` 定义 Hermes 面向用户的语气规范、信息分级展示规则、用户健康保护原则与非操纵设计约束。
   - `SOUL.md` 不得覆盖、削弱或绕过本文件的任何治理规则与安全门禁。
   - 治理规则（AGENTS.md）与交互规范（SOUL.md）发生冲突时，AGENTS.md 优先。
   - `SOUL.md` 的内容变更属于治理边界变更，须经 Workspace ADMIN 人工审批，禁止 AI 自动修改。
   - `SOUL.md` 的当前生效版本号必须记录在 HarnessSnapshot 的 `soulVersion` 字段中，确保每次 Canary 快照可追溯交互规范版本。
